    ...
    methods: {
      init_chat() {
        ...
        socket.on('message', (data) => {
          this.messages.push({ from: 'other', message: data.message, othername: data.othername, nicknameInitials: data.nicknameInitials });
          this.$nextTick(() => {
      ...
      send_message() {
        const to_send = {
          ...
          othername: this.nickname,
          nicknameInitials: this.getInitials(this.nickname)
        };
        
        this.socket.emit('message', to_send);
